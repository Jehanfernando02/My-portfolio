        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <h3 className="flex items-center mb-8 text-2xl font-bold text-primary">
              <div className="w-8 h-8 mr-3 rounded-full bg-gradient-to-r from-purple-500 to-cyan-500" />
              Work Experience
            </h3>
                ...
                >
                  <div className="flex flex-col mb-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <p className="mb-2 text-lg font-semibold text-purple-600 dark:text-purple-400">{exp.company}</p>
                    </div>
                    ...
                    <ul className="space-y-2">
                      {exp.achievements.map((achievement, i) => (
                        <li key={i} className="flex items-start text-sm text-secondary">
                          <div className="w-1.5 h-1.5 bg-cyan-500 rounded-full mt-2 mr-3 flex-shrink-0" />
                          {achievement}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-3 py-1 text-xs font-medium text-purple-600 border rounded-full bg-purple-500/20 dark:text-purple-300 border-purple-500/30"
                      >
                        {tech}
                      </span>
                    ))}
          ...
          >
            <h3 className="flex items-center mb-8 text-2xl font-bold text-primary">
              <div className="w-8 h-8 mr-3 rounded-full bg-gradient-to-r from-cyan-500 to-purple-500" />
              Education
            </h3>
            ...
            <div className="space-y-8">
              {education.map((edu, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.6, delay: index * 0.1 }}
                  viewport={{ once: true }}
                  className="p-8 transition-all surface rounded-2xl hover:bg-cyan-500/5 group"
                >
                  <div className="flex flex-col mb-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      <h4 className="mb-2 text-xl font-bold transition-colors text-primary group-hover:text-gradient">
                      ...
                      <p className="mb-1 text-lg font-semibold text-cyan-600 dark:text-cyan-400">{edu.institution}</p>
